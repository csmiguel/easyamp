...
asv_determination <- function(locus = NULL,
  ...
  filtFs <- sort(list.files("data/intermediate/filtered",
                          pattern = paste0(locus, "_F_filt.fastq.gz"),
                          full.names = TRUE))
  if (sepe == "pe") {
  filtRs <- sort(list.files("data/intermediate/filtered",
                          pattern = paste0(locus, "_R_filt.fastq.gz"),
                          full.names = TRUE))
  assertthat::assert_that(length(filtFs) == length(filtRs))
}
  #error matrices
  ...
  if (sepe == "se") {
  seqtab <-
          dada2::makeSequenceTable(dadaFs) %>%
          dada2::removeBimeraDenovo() #Remove bimeras
        }
        return(seqtab)
      }
